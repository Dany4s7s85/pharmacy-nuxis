...
import { getStoreLevelCount } from "../../../services/businessDashboard";
import { useDispatch } from "react-redux";

const drawerWidth = 240;

export const Sidebar = ({ setOpen, matches, open }) => {
  const { user } = useSelector((state) => state.auth);
  const [levelCount, setLevelCount] = useState({});
  const location = useLocation();
  const history = useNavigate();
  const { pharmacyAllowedPages } = useContext(AuthContext);

  const dispatch = useDispatch();

  const notiCountResponse = useSelector((state) => state?.auth?.notiCount);

  const pharmacyOderResponse = useSelector(
  ...
  useEffect(() => {
    if (user?.store && user?.store?.id) {
      dispatch(
        getStoreLevelCount(function (res) {
  ...
  }, [notiCountResponse, user?.store, pharmacyOderResponse]);
  ...
  const sideBarItems = useMemo(
    () =>
      sideNavData
        .filter(
          ({ file, navigations }) =>
            pharmacyAllowedPages.includes(file) && navigations
        )
        .map((item, index) => item),
    [sideNavData, pharmacyAllowedPages]
  );
  ...
  return (
    ...
    >
      ...
      <Box sx={{ margin: "0px" }}>
        <List className="sidebarContainer css-007">
          {sideBarItems && sideBarItems.length > 0 ? (
            ...
            <ListItem
              ...
              onClick={() =>
                history("/dash/pharmacy-profile", { replace: true })
              }
            >
              <ListItemButton>
                <ListItemIcon>
                ...
                <ListItemText>Pharmacy Profile</ListItemText>
              </ListItemButton>
            </ListItem>
          )}
        </List>
