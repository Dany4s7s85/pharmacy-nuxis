...
import { getPharmacyPurchaseReporting } from "../../../services/pharmacyDashboard";
import {
...
const PurchaseOrderGraph = ({ count }) => {
  ...
  const purchaseOrderLoading = useSelector(
    (state) => state?.pharmacyDashboard?.pharmacyPurchaseReporting?.loading
  );
  ...
  useEffect(() => {
    ...
    dispatch(
      getPharmacyPurchaseReporting("", "", function (res) {
...
export default PurchaseOrderGraph;
