...
import { getPharmacyOrderReporting } from "../../../services/pharmacyDashboard";
import {
...
const SaleOrderGraph = ({ count }) => {
  ...
  const orderLoading = useSelector(
    (state) => state?.pharmacyDashboard?.pharmacyOrderReporting?.loading
  );
  ...
  useEffect(() => {
    ...
    dispatch(
      getPharmacyOrderReporting("", "", function (res) {
        let data = res?.data;
        if (data) {
