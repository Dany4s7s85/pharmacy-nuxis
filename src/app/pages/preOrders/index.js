...
const PreOrders = () => {
  ...
  const response = useSelector((state) => state?.order?.preOrders?.response);
  const loading = useSelector((state) => state?.order?.preOrders?.loading);
  ...
  useEffect(() => {
    dispatch(
      getPreOrders("", "", page, limit, function (res) {
        if (res) {
