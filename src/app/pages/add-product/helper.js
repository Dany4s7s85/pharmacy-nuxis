...
export const Schema = Yup.object().shape({
  ...
  price: Yup.number()
    .max(100, "Price cannot be more than 99")
    .test(
      "decimalDigits",
  ...
  imageCover: Yup.object({
    ...
    full_image: Yup.string(),
  }).required(),
  batch_number: Yup.string().max(255).required("Batch number is required"),
  // fullfillment:Yup.string().required('Fulfillment is required'),
  isAutomatedDiscountApplied: Yup.boolean(),
  discountsArray: Yup.array().of(
    Yup.object().shape({
      month: Yup.number().when("isAutomatedDiscountApplied", {
        is: (isAutomatedDiscountApplied) => true, //just an e.g. you can return a function
        then: Yup.number().required("Month is required"),
        otherwise: Yup.number(),
      }),
      discount: Yup.number().when("isAutomatedDiscountApplied", {
        is: (isAutomatedDiscountApplied) => true, //just an e.g. you can return a function
